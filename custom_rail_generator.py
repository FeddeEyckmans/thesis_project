"""
RECIFE-MILP Compliant Custom Rail Topology Generator Module
===========================================================
...
* Origin:
  ...
  (flatland/envs/rail_generators.py), authored by the Flatland Association.

* This script was decoupled from the global library space and refactored into a local,
  reproducible project module. The core 'SparseRailGen' class was modified to bridge
  the procedural environment generation boundaries with the strict microscopic
  requirements of the RECIFE-MILP optimization engine.

Key Algorithmic Adjustments:
1. Asymmetrical Station Capacity Matrix:
   Introduced the `city_track_counts` parameter array. Instead of assigning a uniform,
   random track count across all urban centers, this feature allows the user to define
   exact track allocations per city index (e.g., [6, 3, 3, 2, 2, 2]).
2. Global Boundary Safeguard Optimization:
   Refactored the `city_radius` calculation to dynamically scale around the maximum bounds
   extracted from the `city_track_counts` matrix, mathematically guaranteeing that dense
   interlocking clusters fit safely within their spatial grid assignments without cell truncation.
3. Outbound Line Maximization:
   Slipped out the random `randint` selection for outward-facing line branches. The network
   now forces maximum connection topology configurations (`min(rails_between_cities,
   nr_of_connection_points)`) to increase macro-routing density and support realistic,
   high-volume timetable scheduling.
"""
...
class SparseRailGen(RailGen):
    ...
    def __init__(self, max_num_cities: int = 2, grid_mode: bool = False, max_rails_between_cities: int = 2,
                 max_rail_pairs_in_city: int = 2, seed: int = None, p_level_free: float = 0, backwards_compatibility_mode: bool = False, city_track_counts: List[int] = None) -> RailGenerator:
        """
    ...
    def generate(self, width: int, height: int, num_agents: int, num_resets: int = 0, np_random: RandomState = None) -> RailGeneratorProduct:
        ...
        if self.city_track_counts is not None:
            self.max_num_cities = len(self.city_track_counts)
            rail_pairs_in_city = int(np.ceil(max(self.city_track_counts) / 2))
        else:
            min_nr_rail_pairs_in_city = 1
            rail_pairs_in_city = min_nr_rail_pairs_in_city if self.max_rail_pairs_in_city < min_nr_rail_pairs_in_city else self.max_rail_pairs_in_city
        ...
        inner_connection_points, outer_connection_points, city_orientations, city_cells = \
            self._generate_city_connection_points(
                ...
                rail_pairs_in_city, np_random=np_random, city_track_counts=self.city_track_counts)
    ...
    def _generate_city_connection_points(self, city_positions: IntVector2DArray, city_radius: int,
                                         ...
                                         city_track_counts: List[int] = None) -> Tuple[
        ...
        for city_idx, city_position in enumerate(city_positions):
            ...
            if city_track_counts is not None and city_idx < len(city_track_counts):
                nr_of_connection_points = city_track_counts[city_idx]
            else:
                nr_of_connection_points = np_random.randint(1, rail_pairs_in_city + 1) * 2

                # Update city_cells met de globale, veilige city_radius
            city_cells.update(
                self._get_cells_in_city(city_position, city_radius, city_orientations[-1], vector_field))
            ...
            # Forceer altijd het maximale aantal sporen in plaats van een willekeurig getal (1 of 2)
            number_of_out_rails = min(rails_between_cities, nr_of_connection_points)
